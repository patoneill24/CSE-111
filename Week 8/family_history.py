...
def print_death_age(people_dict):
    print(f'Age at Death\n{"=" * 20}')
    for people,record in people_dict.items():
        name = record[NAME_INDEX]
        birth_year = record[BIRTH_YEAR_INDEX]
        death_year = record[DEATH_YEAR_INDEX]
        people = f'{name:.<20} {death_year - birth_year}'
        print(people)
...
def count_genders(people_dict):
    ...
    for people,record in people_dict.items():
        ...
        if gender == 'M':
            male_counter += 1
    people = f'Number of males: {male_counter} \nNumber of females: {female_counter}'
    print(people)
...
def print_marriages(marriages_dict, people_dict):
    print(f'Marriages\n{"=" * 20}')
    for people,record in marriages_dict.items():
       husband = record[HUSBAND_KEY_INDEX]
       wife = record[WIFE_KEY_INDEX]
       wedding_year = record[WEDDING_YEAR_INDEX]
       husband_name = people_dict[husband][NAME_INDEX]
       wife_name = people_dict[wife][NAME_INDEX]
       husband_birth_year = people_dict[husband][BIRTH_YEAR_INDEX]
       wife_birth_year = people_dict[wife][BIRTH_YEAR_INDEX]
       husband_age = wedding_year - husband_birth_year
       wife_age = wedding_year- wife_birth_year
       people = f'{husband_name} {husband_age} > {wedding_year} < {wife_name} {wife_age}'
       print(people)
